describe('Scenario 10: dont have yet universal credit and pension credit benefit', function() {
    ...
    it('Benefits - Pension credit', function() {
    ...
    it('Type of Pension Credit - Guarantee credit only', function() {
        cy.get('#question-heading').contains('What type of Pension Credit do you or your partner get?')
        cy.get('#content > div.grid-row > div > form > details > summary > span').click()
        cy.get('#details-content-0').should('be.visible')
        cy.get('#guaranteecreditonly').click()
        cy.get('#next-button').click()
        cy.url().should('include', '/result-claiming-qualifying-pension-credit')
    }),
    ...
    it('Result', function() {
        ...
        cy.go(-1)
        cy.get('#jobcentre-existing-link').should($a => {
            expect($a.attr('href'), 'href')
            expect($a.attr('target'), 'target').to.equal('_blank')
            $a.attr('target', '_self')
        }).click()
